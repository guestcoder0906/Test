...
function updateActionState() {
  const inRange = distanceToSelected() <= config.collectionMeters;
  discoverBtn.disabled = !selectedConcept || !inRange || Boolean(selectedConcept.discovered_name);
  collectBtn.disabled = !selectedConcept || !inRange || !Boolean(selectedConcept.discovered_name);
}

function renderDetails(concept) {
  const name = concept.discovered_name || 'Unknown Concept';
  detailsEl.textContent = `${name} • ${concept.rarity_tier} • ${Math.round(distanceToSelected())}m away`;
}
